...
import { ArrowUpRight, Sparkles, CheckCircle2, AlertCircle, RefreshCw, Calendar } from 'lucide-react';
...
export default function SellingPage() {
  ...
  return (
    <div className="w-full space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-white flex items-center gap-2">
            ...
            Sell Currency Terminal
          </h1>
          <p className="text-slate-400 text-sm mt-1">
            Sell foreign currency or digital assets to Customers or Bankers using dynamic rate rules.
          </p>
        </div>
      </div>
      ...
      {successReceipt && (
        <div className="glass-card rounded-2xl p-6 border border-amber-500/40 bg-amber-950/30 space-y-4 animate-fadeIn">
          <div className="flex items-center gap-3">
            <CheckCircle2 className="w-6 h-6 text-amber-400" />
            <div>
              <h3 className="font-bold text-lg text-amber-300">Sell Trade Executed Successfully!</h3>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 bg-slate-900/80 p-4 rounded-xl text-xs border border-slate-800">
            ...
            <div>
              <span className="text-slate-500 block">Applied Sell Rate</span>
      ...
      <form onSubmit={handleSubmit} className="glass-card rounded-2xl p-6 border border-slate-800 space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="space-y-2">
            <label className="text-xs font-semibold text-slate-300 uppercase tracking-wider">
              Transaction Notes / Delivery Ref
            </label>
            <input
              type="text"
              value={notes}
        ...
        <div className="p-5 rounded-xl bg-slate-900/90 border border-amber-500/30 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <span className="text-xs text-slate-400 uppercase tracking-wider block font-semibold">
              Total Calculated USDT Delivered ({toCurrency})
            </span>
            <div className="text-2xl font-extrabold text-amber-400 mt-0.5">
              {amountReceived} {toCurrency}
            </div>
            <div className="text-xs text-slate-400 mt-1">
              Selling Rate: <span className="text-white font-mono">1 {toCurrency} = ₹{appliedRate}</span>
            </div>
          </div>
          ...
          <button
            type="submit"
            disabled={submitting}
            className="px-6 py-3.5 bg-gradient-to-r from-amber-600 to-orange-600 hover:from-amber-500 hover:to-orange-500 text-white font-bold text-sm rounded-xl shadow-lg shadow-amber-600/30 transition flex items-center justify-center gap-2 disabled:opacity-50"
          >
            {submitting ? (
